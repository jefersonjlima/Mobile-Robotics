...
\begin{frame}[c]{Kalman filter}
    \framesubtitle{Algoritmo do Filtro de Kalman}
...
\begin{frame}[c]{Kalman filter}
    \framesubtitle{Algoritmo do Filtro de Kalman}
    \begin{itemize}
        \item Resolvendo a integral:
        \begin{equation*}
            p(x_t| u_t, x_{t-1})= N\left(x_t; A_tx_{t-1}+ B_tu_t, Q_t\right)
        \end{equation*}   
        \item ou seja:
        \begin{equation*}
            \overline{\text{Bel}}(x_t)  = \displaystyle\int\underbrace{P(x_t|u_t, x_{t-1})}_{\sim N(x_t; A_t x_{t-1}+ B_tu_t, Q_t)} \overbrace{\text{Bel}(x_{t-1})}^{\sim N(x_{t-1}; \mu_{t-1}, \sum {}_{t-1})}\text{d}x_{t-1}
        \end{equation*}
    \end{itemize}
\end{frame}
...
%% TODO write equations and remove it
\foreach\x in{2,3,4,5,6}{
    \begin{frame}[c]{Kalman filter}
        \framesubtitle{Algoritmo do Filtro de Kalman}
        \centering
        \includegraphics[width=0.8\textwidth]{images/tmp\x}
    \end{frame}
}
